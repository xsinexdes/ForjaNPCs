...
public class NpcManager {
    ...
    public void save() {
        ...
        for (ForjaNpc npc : npcs.values()) {
            ...
            cfg.set(base + "lookAtPlayers", npc.isLookAtPlayers());
            cfg.set(base + "lookRadius", npc.getLookRadius());
    ...
    public void load() {
        ...
        for (String key : cfg.getConfigurationSection("npcs").getKeys(false)) {
            try {
                ...
                npc.setLookAtPlayers(cfg.getBoolean(base + "lookAtPlayers", true));
                npc.setLookRadius(cfg.getDouble(base + "lookRadius", 6.0));
